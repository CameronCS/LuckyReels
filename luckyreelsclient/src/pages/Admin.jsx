...
function formatEvent(ev) {
  ...
  switch (ev.game) {
    ...
    case 'blackjack':
      if (d.phase === 'deal') return { icon: '🃏', label: 'DEAL', detail: `bet ${d.bet}`, net: `-${d.bet}`, time }
    ...
    case 'mines':
      if (d.phase === 'start')   return { icon: '💣', label: 'START',   detail: `${d.mineCount} mines · bet ${d.bet}`, net: `-${d.bet}`, time }
      if (d.phase === 'explode') return { icon: '💥', label: 'BOOM',    detail: `${d.revealed} safe`, net, time }
      return { icon: '💎', label: 'CASHOUT', detail: `${d.revealed} safe · ${d.multiplier?.toFixed(2)}×`, net, time }
    case 'crash':
      if (d.phase === 'bet')    return { icon: '🚀', label: 'BET',     detail: `bet ${d.bet}`, net: `-${d.bet}`, time }
...
export default function Admin() {
  ...
  async function login() {
    setLoginErr('')
    if (!username.trim() || !password) { setLoginErr('Enter username and password.'); return }
  ...
  useEffect(() => {
    ...
    const conn = new signalR.HubConnectionBuilder()
      .withUrl('/adminhub', { accessTokenFactory: () => tok })
    ...
    conn.on('PlayerOnlineStatus', ({ playerId, isOnline }) => {
      setOnlineIds(prev => {
        ...
        if (isOnline) next.add(playerId); else next.delete(playerId)
        return next
      })
    })
    ...
    conn.onreconnected(() => {
      ...
      if (watching) conn.invoke('WatchPlayer', watching).catch(() => {})
    })
    conn.onclose(() => {
  ...
  useEffect(() => {
    if (authed) fetchPlayers(page, debouncedSearch)
  ...
  async function toggleExpand(playerId) {
    ...
    if (expanded === playerId) {
      ...
      if (expanded) conn?.invoke('UnwatchPlayer', expanded).catch(() => {})
