...
export default function Horse() {
  ...
  function raceLoop(ts, targetWinner) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (runner) runner.style.left = `${2 + Math.min(pos[i], 100) * 0.885}%`
  ...
  function endRace(targetWinner) {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < 6; i++) {
        if (runnerRefs.current[i]) runnerRefs.current[i].style.left = '2%'
        if (rankRefs.current[i])   { rankRefs.current[i].textContent = ''; rankRefs.current[i].className = 'lane-pos' }
      }
      posRef.current = new Array(6).fill(0)
      setPhase('idle')
  ...
  function startRace() {
    ...
    if (selected < 0) { setStatus('Pick a horse first!'); return }
