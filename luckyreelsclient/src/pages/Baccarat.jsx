...
export default function Baccarat() {
  ...
  async function onResult(msg) {
    ...
    for (const step of seq) {
      await sleep(300)
      if (step.side === 'p') { ph = [...ph, msg.playerHand[step.idx]]; setPlayerHand([...ph]) }
      else                   { bh = [...bh, msg.bankerHand[step.idx]]; setBankerHand([...bh]) }
    }
  ...
  function resultLabel() {
    ...
    if (net > 0) {
      if (result === 'tie') return `TIE! +${net}`
      return result === 'player' ? `PLAYER WINS  +${net}` : `BANKER WINS  +${net}`
    }
    if (net === 0) return 'PUSH — BET RETURNED'
    return result === 'player' ? `Player Wins  −${abs}` : result === 'banker' ? `Banker Wins  −${abs}` : `Tie  −${abs}`
  }
  function resultCls() {
    if (result === 'player') return 'player-win'
    if (result === 'banker') return 'banker-win'
    return 'tie-result'
  }
