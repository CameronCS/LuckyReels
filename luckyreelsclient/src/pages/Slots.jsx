...
function colCount(n) { return n <= 1 ? 1 : n === 2 ? 2 : 3 }
...
export default function Slots() {
  ...
  async function spinOne(id) {
    ...
    if (machines[idx].spinning || globalSpin || tokens < bet) return
    conn.invoke('SpinSlots', idx + 1, bet)
    await spinMachine(id, idx + 1)
  }

  async function spinAll() {
    if (globalSpin || tokens < bet * machines.length) return
    setGlobal(true)
    ...
    if (totalNet > 0) { setMessage(`Total: +${totalNet}`); setMsgCls('win') }
    else { setMessage(''); setMsgCls('neutral') }
  }

  function addMachine() {
    if (machines.length >= MAX) return
