...
public class HorseDataService(App_DBContext context, ActiveTenantService activeTenantService) : BaseDataService(context, activeTenantService), IHorseDataService {
    ...
    public async Task AddHorseLogAsync(LogHorse log, CancellationToken ct = default) {
        await _context.LogHorses.AddAsync(log, ct);
