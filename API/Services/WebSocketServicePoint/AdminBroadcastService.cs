...
public class AdminBroadcastService(
    ...
    public Task BroadcastNotificationAsync(string type, string message)
        => notificationHub.Clients.All.SendAsync(SystemHub.NotificationEvent, new {
            type, message
        });

    public Task NotifyPlayerAsync(Guid playerId, string type, string message)
        => notificationHub.Clients.User(playerId.ToString()).SendAsync(SystemHub.NotificationEvent, new {
            type, message
        });
}
