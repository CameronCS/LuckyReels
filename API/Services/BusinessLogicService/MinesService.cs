...
public class MinesService(IDataLayerService dataLayerService, ActiveTenantService activeTenantService, IHubContext<SystemHub> systemHub, IMapper mapper, IDistributedCache cache, IAdminBroadcastService adminBroadcast) : BaseBusinessServiceWithDataService<IDataLayerService>(dataLayerService, activeTenantService, systemHub, mapper), IMinesService {
    ...
    private async Task<MinesGameState> GetStateAsync(Guid playerId) {
        ...
        return json == null ? throw new InvalidOperationException("No active mines game.") : JsonSerializer.Deserialize<MinesGameState>(json);
    }
    ...
    public async Task<MinesResult> CashoutAsync(Guid playerId, CancellationToken ct = default) {
        ...
        if (state.Revealed.Count == 0) {
            throw new InvalidOperationException("Reveal at least one cell before cashing out.");
        }
