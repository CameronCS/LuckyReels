...
internal static class RouletteEngine
{
    ...
    private static Dictionary<string, (int[] Winners, int Payout)> BuildBets()
    {
    ...
    internal static (int WinNum, int TotalBet, int Net, List<string> WinningKeys) Spin(List<RouletteBet> bets)
    {
        ...
        foreach (RouletteBet bet in bets)
        {
            ...
            if (!Bets.TryGetValue(bet.Key, out (int[] Winners, int Payout) def)) continue;
    ...
    internal static string FormatWinNumber(int n) => n == 37 ? "00" : n.ToString();
}
