...
const HOURLY_BONUS      = 10_000;

async function runHourlyBonus() {
  try {
    const [eligible] = await db.query(
      'SELECT id FROM players WHERE last_bonus_at IS NULL OR last_bonus_at < DATE_SUB(NOW(), INTERVAL 1 HOUR)'
    );
    if (!eligible.length) return;
    const ids = eligible.map(r => r.id);
    await db.query(
      'UPDATE players SET tokens = tokens + ?, last_bonus_at = NOW() WHERE id IN (?)',
      [HOURLY_BONUS, ids]
    );
    for (const { id } of eligible) {
      const p = online.get(id);
      if (p) {
        p.tokens += HOURLY_BONUS;
        send(p.ws, { type: 'bonus', amount: HOURLY_BONUS, tokens: p.tokens });
      }
    }
    if (eligible.length) console.log(`Hourly bonus: awarded ${HOURLY_BONUS} tokens to ${eligible.length} player(s).`);
  } catch (err) {
    console.error('Hourly bonus failed:', err.message);
  }
}
...
function setupWS(server) {
  ...
  // Run bonus check every 5 minutes; awards once per hour per player
  setTimeout(() => {
    runHourlyBonus();
    setInterval(runHourlyBonus, 5 * 60 * 1000);
  }, 10_000);
