...
const db = require('./db');
const { sessions, online, admins, bjGames, minesGames, GAME_ACTIONS, makeRateLimiter } = require('./state');
  ...
  wss.on('connection', (ws) => {
    ...
    async function handleMessage(ws, msg) {
      ...
      if (msg.type === 'getLogs') {
        ...
        const [[slotRows], [bjRows], [rlRows], [hrRows], [bacRows], [mnRows]] = await Promise.all([
        ...
        const logs  = [...slots, ...bj, ...rl, ...hr, ...bac, ...mn].sort((a, b) => b.ts - a.ts).slice(0, limit).map(({ ts, ...rest }) => rest);
